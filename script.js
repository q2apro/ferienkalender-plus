$(document).ready(function()
{
	...
	$('#datepicker').Zebra_DatePicker({
		...
		onSelect: function(view, elements) {
		   window.location.href = '/ferien/?m='+view;
		}
	});
	...
	$('.shlink').click( function(e) {
		...
		if( $('.shlinktxt').length==0) {
			var long_url = $('.shlink').attr('href'); // '/ferien'+window.location.pathname;
	...
	$('#setDE').click( function() {
		...
		if(initdone) {
			setCookie('ferienhd', calcBinFlag(), 60);
		}
	});
	...
	*/
	...
	if($('.ferienm').length>0) {
		var binflag = getCookie('ferienhd');
		var initdone = false;
		if(binflag) {
			if((binflag&1)==1) {
				$('#setDE').trigger('click');
    ...
    $('#music_btn').click( function() {
		$('a[href*="soundcloud.com"]').each(function(){
			var $link = $(this);
			$.getJSON('https://soundcloud.com/oembed?format=js&url=' + $link.attr('href') + '&iframe=true&callback=?', function(response){
				$link.replaceWith(response.html);
			});
		});
		$('#musicembed').show();
		// scroll to in case comment widget is showing up
		$('html, body').animate({scrollTop:$('#musicembed').position().top}, 'slow');
	});
	*/
	
	var added = 'service@'
	var paypallink = 'https://www.paypal.com/cgi-bin/webscr?cmd=_xclick&business='+added+'q2apro.com&item_name=Spende%20Ferienkalender&amount=1&currency_code=EUR';
	$('#paypalurl').attr('href', paypallink);
